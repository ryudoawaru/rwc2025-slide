...
# RubyConf Taiwan x COSCUP 2025 からの発想
...
**3つのフェーズ:**
1. **Lexical Analysis**: トークン分割と特殊記号識別
2. **Syntax Analysis**: AST構築とパターンマッチング
3. **Semantic Analysis**: 変換処理とバランス検証
...
この実装は、3つのフェーズで構成されています：

1つ目は、Lexical Analysis。トークン分割と特殊記号の識別を行います。
2つ目は、Syntax Analysis。ASTを構築し、パターンマッチングを行います。
3つ目は、Semantic Analysis。変換処理とバランス検証を行います。
...
左側を見てください。
RomanParser が出力した roman_array があります。
...
```bash
$ ruby test_with_stats.rb

Test Results / テスト結果統計
================================
Total tests / 総テスト筆数: 3333
Passed / 通過: 3331 (99.94%)
Failed / 失敗: 2 (0.06%)

Parser succeeded / Parser成功:
  457 (13.71%)
Fallback used / Fallback使用:
  2874 (86.23%)
```

**重要なポイント:**
- ✅ **99.94% 正答率**
- ✅ 13.71% は Parser のみで成功
- ✅ 86.23% は Fallback 使用
- ✅ 実用性と理論の両立
...
<!--
Speaker Note:
ここまで、RomanParser の理論と実装について説明してきました。
しかし、理論だけではなく、実際に動くコードを見ていただきたい。

そこで、この RomanParser の実装を GitHub で公開しています。

【リポジトリ構成】

リポジトリには、3つの主要なファイルがあります：

1つ目は、roman_parser.rb。
これは、今日説明した 3-Phase Parser の完全な実装です。
Parslet を使った Lexical Analysis、Syntax Analysis、Semantic Analysis、
すべてが含まれています。

2つ目は、test_with_stats.rb。
これは、統計情報付きのテストスクリプトです。
Parser の成功率、Fallback の使用率など、詳細な分析ができます。

3つ目は、corpora-test-data.json。
これは、実際のプロジェクトから抽出した 3,333 件のコーパスデータです。
...
総テスト件数は 3,333 件。
そのうち、3,331 件が正解。通過率は 99.94% です。
失敗したのは、わずか 2 件のみ。

興味深いのは、Parser の成功率です。

457 件（13.71%）は、Parser だけで正しく解析できました。
2,874 件（86.23%）は、Fallback を使用しました。

しかし、重要なのは、Fallback を使っても最終的な出力は正しい、ということです。

これは、実用性と理論の両立を示しています。

Parser で処理できるケースは Parser で処理し、
複雑すぎるケースは Fallback で処理する。

この戦略により、99.94% という高い正答率を達成できたのです。

...
2023年(ねん)6月(がつ)、松江市(まつえし)から
Ruby を通(つう)じた日台交流(にったいこうりゅう)についてメールをいただきました。

実際(じっさい)に会(あ)って、お互(たが)いの考(かんが)えを共有(きょうゆう)し、
協力(きょうりょく)の可能性(かのうせい)を探(さぐ)りました。

同年(どうねん)11月(がつ)、RWC 2023 で上定市長(かみさだしちょう)と市役所(しやくしょ)で会談(かいだん)し、
そして RWC で再(ふたた)び登壇(とうだん)しました。

2024年(ねん)2月(がつ)、市長(しちょう)が台北(たいぺい)の 5xRuby Office を訪問(ほうもん)しました。

今年8月(がつ)、RubyConf Taiwan で正式(せいしき)に覚書(おぼえがき)を締結(ていけつ)しました。
...
次(つぎ)に、プロセスの問題(もんだい)があります。
RFP が不十分(ふじゅうぶん)で、
担当者(たんとうしゃ)自身(じしん)がソフトウェアシステムについて
十分(じゅうぶん)な理解(りかい)を持(も)っていないことがあります。
これが実務(じつむ)との乖離(かいり)を招(まね)きます。
...
<!--
Speaker Note:
日本語(にほんご)の漢字(かんじ)が平仮名(ひらがな)に対応(たいおう)するように、
台湾語(たいわんご)の漢字(かんじ)も POJ に対応(たいおう)します。

平仮名(ひらがな)も POJ も、どちらも「音標(おんぴょう)」です。

だから、漢字(かんじ)と音標(おんぴょう)を正確(せいかく)に対応(たいおう)させる
「分詞(ぶんし)アライメント処理(しょり)」が必要(ひつよう)になります。
-->
...
<!--
Speaker Note:
Phase 1 では、漢字(かんじ)と POJ を正規化(せいきか)します。

漢字(かんじ)と POJ は、それぞれ数十種類(すうじゅうしゅるい)の正規表現(せいきひょうげん)を使(つか)って文字列(もじれつ)を処理(しょり)します。

重要(じゅうよう)なのは、POJ のハイフンを保持(ほじ)することです。
後(あと)の Phase で音節(おんせつ)カウントに使(つか)います。
-->
...
まず、Regular Expressionで文字(もじ)をスキャンし、CJK 文字(もじ)と非(ひ) CJK 文字(もじ)を区別(くべつ)します。
...
<!--
Speaker Note:
Phase 2-2 は非常(ひじょう)にシンプルです。
Phase 1 で記号(きごう)を分離(ぶんり)したので、スペースで分割(ぶんかつ)するだけです。

重要(じゅうよう)な設計(せっけい)は、ハイフンでは分割(ぶんかつ)しないことです。
ハイフンは音節(おんせつ)の境界(きょうかい)を示(しめ)すマーカーです。

画面(がめん)の例(れい)では、
最初(さいしょ)の単語(たんご)に 2 つのハイフンがあるので、3 音節(おんせつ)です。
-->
...
<!--
Speaker Note:
Phase 3 は対齊(たいせい)と検証(けんしょう)です。

音節数(おんせつすう)で漢字(かんじ)とマッチングします。
3 音節(おんせつ)なら漢字(かんじ) 3 文字(もじ)に対応(たいおう)します。

Edge Case は、kanji 側(がわ)の Roman 文字(もじ)をそのまま対応(たいおう)させます。

最後(さいご)に、transpose で配列(はいれつ)を分離(ぶんり)し、
3 つの条件(じょうけん)で平衡性(へいこうせい)を検証(けんしょう)します。
-->
...
<!--
Speaker Note:
近年(きんねん)、Ruby Parser の発展(はってん)により、Syntax Parser に関(かん)する発表(はっぴょう)に触(ふ)れる機会(きかい)がありました。

今年(ことし)の RubyConf Taiwan で、金子(かねこ)さんの「Understanding Ruby Grammar Through Conflicts」を聞(き)いた後(あと)、
気(き)づきました。

私(わたし)が今回(こんかい)のプロジェクトで実装(じっそう)した分詞処理(ぶんししょり)も、
...
しかし、他(ほか)の人(ひと)の発表(はっぴょう)に触発(しょくはつ)されて、
この可能性(かのうせい)に気(き)づき、
今回(こんかい)の発表(はっぴょう)と研究(けんきゅう)が生(う)まれました。-->
...
Ruby で Parser を書(か)くための Gem を探(さが)しました。

そこで見(み)つけたのが、Parslet です。

Parslet は、PEG Parser を Ruby の DSL で書(か)けるライブラリです。

重要(じゅうよう)なのは、Parslet の設計思想(せっけいしそう)です。

Parslet は、開発者(かいはつしゃ)に自然(しぜん)と 3 つの Phase を意識(いしき)させる設計(せっけい)になっています。

Lexical Analysis、Syntax Analysis、Semantic Analysis。

この構造(こうぞう)が、まさに私(わたし)が学(まな)んだ Parser の 3 段階処理(だんかいしょり)と一致(いっち)していました。-->
...
Parsing Expression Grammar では、最初(さいしょ)にマッチしたパターンが採用(さいよう)されます。
...
<!--
Speaker Note:
GSUB 方式(ほうしき)と Parslet 方式(ほうしき)を比較(ひかく)します。

左側(ひだりがわ)の GSUB 方式(ほうしき)は、
前(まえ)のページで説明(せつめい)した実装(じっそう)です。
記号(きごう)の前後(ぜんご)に空白(くうはく)を挿入(そうにゅう)し、
その後(あと)スペースで分割(ぶんかつ)します。

右側(みぎがわ)の Parslet では、
記号(きごう)を Token として直接(ちょくせつ)認識(にんしき)します。

結果(けっか)は構造化(こうぞうか)された AST として出力(しゅつりょく)され、
split が不要(ふよう)になります。-->
...
<!--
Speaker Note:
ハイフン処理(しょり)による音節数(おんせつすう)の計算(けいさん)を比較(ひかく)します。

左側(ひだりがわ)の GSUB 方式(ほうしき)は、
前述(ぜんじゅつ)の実装(じっそう)と同(おな)じです。
ハイフンで音節数(おんせつすう)をカウントし、
音節数(おんせつすう)分の漢字(かんじ)を取得(しゅとく)します。

重要(じゅうよう)なポイントは右側(みぎがわ)の Parslet です。

Parser がハイフン付(つ)き単語(たんご)を
1 つの Token として認識(にんしき)し、
音節構造(おんせつこうぞう)を自動(じどう)で保持(ほじ)します。

Phase 3 で音節数(おんせつすう)を計算(けいさん)すれば、
対応(たいおう)する漢字数(かんじすう)も自動(じどう)で決(き)まります。

Parser による構造化(こうぞうか)の利点(りてん)です。-->
...
左側(ひだりがわ)は Ruby の Prism Parser です。
右側(みぎがわ)は実験的(じっけんてき)に作(つく)った RomanParserPure です。

もちろん、Ruby Parser ほど複雑(ふくざつ)ではありません。
これは実験的(じっけんてき)な実装(じっそう)です。

しかし、両方(りょうほう)とも同(おな)じ 3-phase の構造(こうぞう)を持(も)っています。

Parser の考(かんが)え方(かた)は、プログラミング言語(げんご)だけでなく、
自然言語(しぜんげんご)の処理(しょり)にも応用(おうよう)できる。

これが、この実験(じっけん)から学(まな)んだことです。-->
...
<!--
Speaker Note:
この RomanParserPure の実装(じっそう)を GitHub で公開(こうかい)しています。

リポジトリには、Parser の完全(かんぜん)な実装(じっそう)、テストスクリプト、
そして 3,000 件(けん)の実際(じっさい)のコーパスデータが含(ふく)まれています。

テスト結果(けっか)は、3,000 件(けん)すべて正解(せいかい)。通過率(つうかりつ) 100% です。

ぜひ、GitHub から clone して、実際(じっさい)に試(ため)してみてください。-->
...
<!--
Speaker Note:
今日説明(きょうせつめい)した分詞(ぶんし)アライメント処理(しょり)を活用(かつよう)した検索(けんさく)システムです。
漢字(かんじ)と台羅(たいら)が正確(せいかく)に対応(たいおう)し、音声(おんせい)も再生(さいせい)できます。
-->
...
<!--
Speaker Note:
台湾(たいわん)の小中学校(しょうちゅうがっこう)の教科書語彙(きょうかしょごい)データベースです。
教育現場(きょういくげんば)での実用性(じつようせい)を重視(じゅうし)しています。
-->
...
<!--
Speaker Note:
台湾語(たいわんご)の文法(ぶんぽう)パターンを体系化(たいけいか)したデータベースです。
これら3つの機能(きのう)の基盤(きばん)が、今日紹介(きょうしょうかい)した分詞(ぶんし)アライメント処理技術(しょりぎじゅつ)です。
-->
...
今回(こんかい) RubyWorld の1階展示場(かいてんじじょう)では、
Ruby Taiwan コミュニティと RUBYCITY MATSUE が
共同(きょうどう)でブースを出展(しゅってん)しております。
ぜひお立(た)ち寄(よ)りください。

松江市(まつえし)との MOU を通(つう)じて、日台(にったい)の Ruby コミュニティの交流(こうりゅう)を深(ふか)めていきたいと思(おも)っています。
